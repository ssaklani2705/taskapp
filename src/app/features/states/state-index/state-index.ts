...
export class StateIndex {
  ...
  filterKey = SESSION_KEYS.STATE_FILTER;
  ...
  ngOnInit(): void {
    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      //   } catch (error) {
      //     console.error('Failed to restore state filter:', error);
      //   }
      // }
  ...
  getTaskCategoryDetails(): void {
    this.dataprovider.getStateList(this.page, this.size, this.statusIndex, this.search).subscribe({
      next: (response: any) => {
        this.apiResponseState = response?.data || {};
  ...
  onDeleteState(stateId: number): void {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.dataprovider.deleteState(payload).subscribe({
          next: (response: any) => {
            if (response.success) {
  ...
  // viewTaskCategory(stateId: number): void {
  //   const filterState = {
  //     currentPage: this.currentPage,
  //     statusIndex: this.statusIndex,
  //     searchText: this.searchQuery.trim(),
  ...
  //   this.router.navigate(['/view-state', stateId], {
  ...
  // editTaskCategory(stateId: number): void {
  //   const filterState = {
  //     currentPage: this.currentPage,
  //     statusIndex: this.statusIndex,
  //     searchText: this.searchQuery.trim(),
  ...
  get recordSummary(): string {
    const totalRecords = this.apiResponseState?.totalElements || 0;
  ...
  get totalPages(): number {
    const total = this.apiResponseState?.totalElements || 0;
