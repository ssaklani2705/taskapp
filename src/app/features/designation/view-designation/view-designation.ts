...
export class ViewDesignation implements OnInit {
  ...
  ngOnInit(): void {
    this.designationId = +this.route.snapshot.paramMap.get('designationId')!;
  ...
  getDesignationDetails(): void {
    this.dataprovider.getDesigmationById(this.designationId).subscribe({
      next: (response) => {
        if (response && response.success && response.data) {
          ...
          console.error('Designation details not found');
        }
      },
